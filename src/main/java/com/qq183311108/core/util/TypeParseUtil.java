package com.qq183311108.core.util;
...
import com.qq183311108.core.DataType;
...
public final class TypeParseUtil {
	...
	public static final Object convert(Object value, String type, String format) {
		...
		if (value == null) {
			...
			throw new MyException(CommErrCode.ParamTyepConvertError, String.format(support, value.getClass().getName(), type));
		}
	}
	...
	private static Object bool2Obj(Object value, String type) {
		...
		if ("Boolean".equalsIgnoreCase(type) || DataType.BOOLEAN.equalsIgnoreCase(type)) {
			...
			throw new MyException(CommErrCode.ParamTyepConvertError, String.format(support, fromType, type));
		}
	}
	...
	private static Object sqlDate2Obj(Object value, String type, String format) {
		...
		if ("String".equalsIgnoreCase(type) || DataType.STRING.equalsIgnoreCase(type)) {
			...
			throw new MyException(CommErrCode.ParamTyepConvertError, "Conversion from " + fromType + " to " + type + " not currently supported");
		} else if ("Timestamp".equalsIgnoreCase(type) || DataType.TIMESTAMP.equalsIgnoreCase(type)) {
				...
				if (format == null || format.length() == 0)
					try {
						...
						try {
							...
							throw new MyException(CommErrCode.ParamTyepConvertError, String.format(message, str, type), e);
						}
					}
				SimpleDateFormat sdf = new SimpleDateFormat(format);
